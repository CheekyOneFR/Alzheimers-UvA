...
other_variables = c("Age","ADNI_MEM","ADNI_EF","diagnosis","Sex","EDUC","APOE4","Amy_Stage",
                    ...
                    "AVDEL30MIN", "AVDELTOT") ### Now includes the other cognitive tests
# , "LIMMTOTAL", "CDR (CDGLOBAL) at scan"
# include these ^ two after some investigation into their relationship.
...
### In addition with the new variables these ones are character (string) values when they should
### be numeric
### LDELTOTAL, BNTSPONT, BNTTOTAL, TRAASCOR, TRABSCOR, TRAB-A, AVDEL30MIN, AVDELTOT, LIMMTOTAL, CDR (CDGLOBAL) at scan
...
data_2$LDELTOTAL = as.numeric(data_2$LDELTOTAL)
data_2$BNTSPONT = as.numeric(data_2$BNTSPONT)
data_2$BNTTOTAL = as.numeric(data_2$BNTTOTAL)
data_2$TRAASCOR = as.numeric(data_2$TRAASCOR)
data_2$TRABSCOR = as.numeric(data_2$TRABSCOR)
data_2$`TRAB-A` = as.numeric(data_2$`TRAB-A`)
data_2$AVDEL30MIN = as.numeric(data_2$AVDEL30MIN)
data_2$AVDELTOT = as.numeric(data_2$AVDELTOT)
#data_2$LIMMTOTAL = as.numeric(data_2$LIMMTOTAL)
#data_2$`CDR (CDGLOBAL) at scan` = as.numeric(data_2$`CDR (CDGLOBAL) at scan`)
...
### For some reason LIMMTOTAL and CDR seem to be mutually exclusive? So I'll circle back to including those later.
...
### Names of the cognitive tests go in between the "Amy-stage" variable and the brain imaging variables
